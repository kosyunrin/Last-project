...
public class BeamControl : MonoBehaviour
{
    ...
    private Transform LaserManager = null;
    private int LaserID = 0;
    ...
    public void Init()
    {
        List<GameObject> _LaserMangaer=null;
        _LaserMangaer = new List<GameObject>();

        LaserManager = GameObject.Find("LaserManger").transform;
        cRayTest = new RayTest();
        var objL = GameObject.Instantiate(objLinePerfab);
        objL.transform.localScale = transform.localScale;
        objL.transform.SetParent(LaserManager);
        _LaserMangaer.Add(objL);
        objLinePerfab = objL;
        ...
        objP.transform.SetParent(LaserManager);
        _LaserMangaer.Add(objP);
        objParticlePerfab = objP;
        rParticleRender = objP.GetComponent<Renderer>();
        rParticleRender.enabled = false;
        LaserID =
        this.transform.parent.GetComponent<ENEMY_AI.skEnemyController>().poolID;
        LaserManager.GetComponent<LaserManagerSK>().Laser.Add(LaserID, _LaserMangaer);

        Clear();
    }
    public void RestWeapon()
    {
        var d = LaserManager.GetComponent<LaserManagerSK>().Laser;
        var s = d[LaserID];
